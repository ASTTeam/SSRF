...
本项目创建于2022年3月3日，最近的一次更新时间为2022年10月19日。本项目会持续更新，直到海枯石烂！
...
三、SSRF漏洞分类
- 按照是否回显分：
  - Basic SSRF：因此在服务器获取攻击者要求的URL后，它将把响应发送回攻击者返回结果到客户端，如传送一个网址，会返回这个网址的界面或对应的 html 代码。
  - Blind SSRF：响应中不返回服务器中的任何信息。
- Semi SSRF：不会返回相关结果请求的所有详细信息，但是会暴露一些数据信息。
- 按照程序语言分：
  - PHP SSRF
  - Java SSRF
  - ASPX SSRF
